...
class TestMpost
  ...
  def create
    params = to_params(:time, :lng, :lat, :lerror)
    params.merge!(:devs=>((peers.concat(leaks).concat([user]).map {|peer| peer.dev}).join(",")))
    params.merge!(:user_dev=>user.dev, :user_id=>user.id)
