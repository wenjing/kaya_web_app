...
relation.populate_from_cluster(cluster, &processer.method(:coeff_calculator))
relation.proceed(&processer.method(:pass_coeff_criteria_normal?))
mposts = relation.seeded.keys
cluster = pool.split_cluster(mposts, cluster)
cluster.meet = processer.method(:create_meet_from_mposts).call(mposts)
pool.move_to_processed_clusters(cluster)
pool.dump_debug(:pool)
puts cluster.meet.inspect

puts "========================="
mpost = q4
meet = cluster.meet
relation = MeetRelation.new
relation.populate_from_meet_for_mposts(meet, [mpost], 1000000.0, &processer.method(:coeff_calculator))
relation.proceed(&processer.method(:pass_coeff_criteria_simple?))
processer.method(:assign_mpost_to_meet).call(mpost, meet)
puts meet.inspect
