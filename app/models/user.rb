...
class User < ActiveRecord::Base
  ...
  has_many :deleted_meets, :class_name => "Meet",
                   :through => :mposts, :uniq => true,
                   :conditions => ['mposts.status = ?', 1]
  has_many :pending_meets, :class_name => "Meet",
                   :through => :mposts, :uniq => true,
                   :conditions => ['mposts.status = ?', 2]
  has_many :pending_invitations, :class_name => "Invitation",
                   :through => :mposts, :uniq => true,
                   :conditions => ['mposts.status = ?', 2]
  has_many :solo_meets, :through => :mposts, :uniq => true,
             :conditions => ['mposts.status = ? AND meets.type = ?', 0, 1]
  has_many :private_meets, :through => :mposts, :uniq => true,
             :conditions => ['mposts.status = ? AND meets.type = ?', 0, 2]
  has_many :group_meets, :through => :mposts, :uniq => true,
             :conditions => ['mposts.status = ? AND meets.type = ?', 0, 3]
  ...
  validates :name,  :presence => true,
                    :length   => { :maximum => 50 }, :unless => :exclusive_procudure?
  validates :email, :presence   => true,
                    :format     => { :with => email_regex }, :uniqueness  => true
  validates :password, :presence => true, :confirmation => true,
                       :length => { :within => 6..40 }, :unless => :exclusive_procudure
  validates_attachment_size :photo, :less_than => 1.megabyte
  validates_attachment_content_type :photo, 
    :content_type => ['image/jpg', 'image/jpeg', 'image/gif', 'image/png'] 

  before_save {|record| record.email.strip!.downcase! if (record.email.present? && !exclusive_procudure?)}
  before_save {|record| record.encrypt_password if !exclusive_procudure?}
  ...
  def delete
    ...
    email = "DELETED_#{Time.now.getutc.iso8601}_#{email}"
  end
  def recovery_to(to_status)
    status = to_status
  end
  ...
  def has_password?(submitted_password)
    ...
    if (!matched && pending? && temp_password.present?)
      ...
      if matched # temp_password is only one time use
        opt_lock_protected {
...
# end
  ...
  def top_chatters(limit)
    ...
    # of meet time.
    return Chatter.related_to(self).limit(limit).to_a
  end

  # Return most recent met comments from time_ago
  def recent_chatters(time_ago)
    time_after = Time.now-time_ago
    return Chatter.related_to(self).where("chatters.created_at >= ?", time_after).to_a
  end
  ...
  def meets_with(user, type=nil)
    if user.id == id
      ...
      return meets.to_a.select {|meet| meet.include_user?(user) && meet.of_type?(type)}
    else
      # A bit tricky. If use meets directly, it may involve unnecessary DB load of meet info.
      # Use meets_id and users_id where possible to avoid loading full meet info from DB
      user_meet_ids = user.meet_ids.to_set
      meet_ids_with = safe_meet_ids.select {|meet_id| user_meet_ids.include?(meet_id)}
      return (Meet.find(meet_ids_with) || []).select {|meet| meet.of_type?(type)}
    end
  end

  def safe_meet_ids
    return meet_ids.to_a.compact
  end
  def safe_pending_meet_ids
    active_meet_ids = safe_meet_ids.to_set
    return pending_meet_ids.select {|v| !v.nil? && !active_meet_ids.include?(v)}
  end
  def meets_count
    return safe_meet_ids.size
  end
  def pending_meets_count
    return safe_pending_meet_ids.size
  end
  def safe_pending_meets
    return pending_meets.to_a.select {|meet| !meet.include_user?(self)}
  end

  def dev
    return "#{name_or_email}:#{id?id:""}"
  end
